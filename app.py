...
# -------------------------------------------
# NEW: User-friendly variable labels
# -------------------------------------------
VARIABLE_LABELS = {
...
# NEW: Recode numeric EDUC to categories
def recode_educ(x):
    if x <= 8:
...
def render_cluster_summary(cluster_id):
    ...
    # Extract median stats for this cluster
    median_stats = cluster_summary_median.loc[cluster_id].copy()

    # Convert coded variables → readable
    ...
    # Recode EDUC categories
    median_stats['EDUC'] = recode_educ(round(median_stats['EDUC']))

    # Replace column names with readable labels
    renamed_stats = median_stats.rename(index=VARIABLE_LABELS)

    st.markdown("### **Median Household Characteristics**")
    st.dataframe(styled_table(renamed_stats), use_container_width=True)
...
- Education-Related Installment Debt
...
*While net worth and income contribute to the cluster definitions themselves, they are also essential inputs for the predictive model that maps an individual household’s financial data to the most fitting cluster, enabling a more precise profile classification.*
...
with tab4:
    ...
    cluster_label_map = {
        "Cluster 1: Older, Very Wealthy Households": 1,
        "Cluster 2: Low-Wealth, Lower-Income, Low-Debt Households": 2,
        "Cluster 3: Younger, Moderate-Income, High-Debt Households": 3
    }

    cluster_choice = st.selectbox(
        "Select Your Cluster",
        options=list(cluster_label_map.keys())
    )

    cluster = cluster_label_map[cluster_choice]
    ...
    # -------------------------------------------
    # INCOME + NET WORTH WITH UNKNOWN OPTION
    # -------------------------------------------

    st.markdown("""
    <small>ℹ️ <strong>Tip:</strong> If you select <em>Unknown</em> for income or net worth, 
    the app will automatically substitute the <strong>median value for your cluster</strong>.</small>
    ...
    if income_choice == "Enter Amount":
        ...
        income = None  # replaced by median later
    ...
    if networth_choice == "Enter Amount":
        ...
        networth = None  # replaced by median later

    # -------------------------------------------
    # PREDICT BUTTON + MODEL PIPELINE
    # -------------------------------------------
    if st.button("Predict Education Category"):

        # Convert user-friendly labels → codes
        married_code = {v: k for k, v in married_map.items()}[married]
        race_code = {v: k for k, v in race_map.items()}[race]

        # SUBSTITUTION OF MEDIANS WHEN UNKNOWN
        if income is None:
            income = cluster_stats.loc[cluster, "median_income"]  # <-- ensure cluster_stats exists
        if networth is None:
            networth = cluster_stats.loc[cluster, "median_networth"]

        # Log transforms (protect against negative NW)
...
demographic and financial characteristics. Your predicted category reflects statistical
patterns among people with comparable **income, wealth, race, marital status, and cluster membership**.
""")
